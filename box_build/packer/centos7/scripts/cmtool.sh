...
# to set variables in pairs with Packer (and Packer does not support　multi-value variables).
...
install_ansible()
{
  ...
  if [[ ${CM_VERSION:-} == 'latest' ]]; then
    ...
    yum install -y --enablerepo=epel "ansible-${CM_VERSION}"
  fi
}
...
install_puppet()
{
  ...
  if [[ ${CM_VERSION:-} == 'latest' ]]; then
    ...
    yum install -y "puppet-${CM_VERSION}"
  fi
}
